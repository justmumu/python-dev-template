...
TEMPLATE_REPOSITORY = "justmumu/python-dev-template"
...
def _staging_directory() -> Generator[Path, None, None]:
    staging = Path(tempfile.mkdtemp(prefix="python-bootstrap-"))
    preserve = False
    try:
        yield staging
...
def _validate_target(repo: Path, repository: str) -> tuple[str, str]:
    if repository.count("/") != 1:
        raise ValueError("Repository must be owner/project.")
...
def bootstrap(
    ...
    *,
    ...
    data = {
        ...
        "repo_owner": owner,
        "description": description or f"Python application for {project}.",
